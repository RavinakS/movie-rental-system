...
exports.buyMovie = async (req, res) =>{
    ...
    if(movieName === undefined){
        return res.send({status_code: 400, error_msg: "Please provide the movie name."});
    }
    ...
    try{
        ...
        if(availableRents === null){
            return res.send({status_code: 404, message: "Could not find the movie."});
        }
        ...
        if(availableRents.avalCD > 0){
            ...
            if(!req.movieRentExist){
                ...
                return res.send({status_code: 200, message: "Congratulations!! Enjoy your movie."});
            }
            return res.send({status_code: 403, message: "You Already have this movie, check your rents."})  
        }
        console.log(availableRents);
        res.send({status_code: 403, message: "Oops! Currently this movie is not available for rents."});
...
exports.viewUserRents = async (req, res) =>{
    ...
    if(user_id === undefined){
        return res.send({status_code: 400, error_msg: "Please provide the user_id."});
    }
    try{
        if(!req.admin){
            return res.send({status_code: 401, message: "Access denied."});
        }
        getUserDetails = await userDetailsById(user_id); 
        if(getUserDetails.length === 0){
            return res.send({status_code: 404, message: "User not exist."});
        }else if(getUserDetails[0].rent === 0){
            return res.send({status_code: 200, rents: "0 rents."})
        }
        all_rents = await findRentsByUserID(user_id);
        res.send({status_code: 200, No_of_rents: `${getUserDetails[0].rent}`, rents: all_rents});
    }catch(err){
        console.log(err);
        res.send(err);
    }
